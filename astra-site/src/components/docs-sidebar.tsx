import { component, store, dynamic } from '@astrajs/core';
import { Link, params } from '@astrajs/router';

interface DocSection {
  title: string;
  items: { label: string; href: string }[];
}

const docSections: DocSection[] = [
  {
    title: 'Introducción',
    items: [
      { label: '¿Qué es AstraJS?', href: '/docs/introduction' },
      { label: 'Instalación', href: '/docs/introduction' },
      { label: 'Primeros pasos', href: '/docs/introduction' },
      { label: 'Conceptos clave', href: '/docs/introduction' },
    ],
  },
  {
    title: 'Fundamentos',
    items: [
      { label: 'Componentes', href: '/docs/fundamentals' },
      { label: 'Reactividad con store', href: '/docs/fundamentals' },
      { label: 'JSX sin VDOM', href: '/docs/fundamentals' },
      { label: 'Estilos con css', href: '/docs/fundamentals' },
      { label: 'Eventos resumibles', href: '/docs/fundamentals' },
    ],
  },
  {
    title: 'Server & Data',
    items: [
      { label: 'server', href: '/docs/server-data' },
      { label: 'Tipos pre-build vs dynamic', href: '/docs/server-data' },
      { label: 'Revalidate & Caching', href: '/docs/server-data' },
      { label: 'autoSync y ETAGS', href: '/docs/server-data' },
    ],
  },
  {
    title: 'Router',
    items: [
      { label: 'Rutas y <Outlet />', href: '/docs/router' },
      { label: 'Layouts anidados', href: '/docs/router' },
      { label: 'Navegación', href: '/docs/router' },
      { label: 'View Transitions API', href: '/docs/router' },
    ],
  },
  {
    title: 'Renderizado',
    items: [
      { label: 'SSR', href: '/docs/rendering' },
      { label: 'SSG', href: '/docs/rendering' },
      { label: 'ISR', href: '/docs/rendering' },
      { label: 'Resumibilidad', href: '/docs/rendering' },
    ],
  },
  {
    title: 'Comparativa',
    items: [
      { label: 'React vs AstraJS', href: '/docs/comparison' },
      { label: 'Vue.js vs AstraJS', href: '/docs/comparison' },
      { label: 'Angular vs AstraJS', href: '/docs/comparison' },
    ],
  },
  {
    title: 'Avanzado',
    items: [
      { label: 'Compilador AST', href: '/docs/advanced' },
      { label: 'Inferencia de tipos', href: '/docs/advanced' },
      { label: 'Integración con Vite', href: '/docs/advanced' },
      { label: 'Despliegue', href: '/docs/advanced' },
    ],
  },
];

export const DocSidebar = component(() => {
  const state = store({ activeSection: '' });

  const style = `
    .docs-sidebar{position:fixed;top:64px;left:0;bottom:0;width:260px;background:#060b14;border-right:1px solid rgba(255,255,255,.06);overflow-y:auto;padding:24px 0 40px;z-index:50}
    .docs-sidebar-section{margin-bottom:8px}
    .docs-sidebar-title{padding:8px 28px;font-size:.68rem;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.08em}
    .docs-sidebar-item{display:block;padding:7px 28px;font-size:.8rem;color:#94a3b8;font-weight:500;transition:color .12s,background .12s;border-left:2px solid transparent}
    .docs-sidebar-item:hover{color:#e2e8f0;background:rgba(255,255,255,.02)}
    .docs-sidebar-item.active{color:#b84cff;background:rgba(184,76,255,.06);border-left-color:#b84cff}
    .docs-sidebar-footer{padding:20px 28px;border-top:1px solid rgba(255,255,255,.06);margin-top:16px}
    .docs-sidebar-footer a{display:flex;align-items:center;gap:8px;font-size:.78rem;color:#64748b;font-weight:500;transition:color .15s}
    .docs-sidebar-footer a:hover{color:#e2e8f0}
    @media(max-width:960px){
      .docs-sidebar{display:none}
    }
  `;

  return (
    <aside class="docs-sidebar">
      <style>{style}</style>
      <nav>
        {docSections.map(section => (
          <div class="docs-sidebar-section">
            <div class="docs-sidebar-title">{section.title}</div>
            {section.items.map(item => (
              <Link href={item.href} class="docs-sidebar-item">
                {item.label}
              </Link>
            ))}
          </div>
        ))}
      </nav>
      <div class="docs-sidebar-footer">
        <a href="https://github.com" target="_blank" rel="noopener">
          <span>⭐</span> 26.1k GitHub
        </a>
      </div>
    </aside>
  );
});
